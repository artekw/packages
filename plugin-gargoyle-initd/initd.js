...
function resetData()
{
	...
	var columnNames = ['Usługa', 'Autostart', 'Start', 'Restart', 'Stop'];
	...
	var blockedServices = ['network', 'boot', 'dropbear', 'telnet', 'sysctl', 'done'];
...
function startService()
{
	...
	var param = getParameterDefinition("commands", commands) + "&" + getParameterDefinition("hash", document.cookie.replace(/^.*hash=/,"").replace(/[\t ;]+.*$/, ""));
	setControlsEnabled(false, true, "Próba uruchomiania "+ service);
        var stateChangeFunction = function(req)
        {
			if(req.readyState == 4)
			{
...
function stopService()
{
	...
	commands = executeCommand.join("\n");
	var param = getParameterDefinition("commands", commands) + "&" + getParameterDefinition("hash", document.cookie.replace(/^.*hash=/,"").replace(/[\t ;]+.*$/, ""));
	setControlsEnabled(false, true, "Próba zatrzymania "+ service);
	var stateChangeFunction = function(req)
        {
			if(req.readyState == 4)
			{
...
function restartService()
{
	...
	commands = executeCommand.join("\n");
	var param = getParameterDefinition("commands", commands) + "&" + getParameterDefinition("hash", document.cookie.replace(/^.*hash=/,"").replace(/[\t ;]+.*$/, ""));
	setControlsEnabled(false, true, "Próba zrestartowania "+ service);
	var stateChangeFunction = function(req)
	{
		if(req.readyState == 4)
		{
